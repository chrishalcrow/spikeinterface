...
class ComputeTemplateMetrics(BaseMetricExtension):
    ...
    def _handle_backward_compatibility_on_load(self):
        ...
        if "depth_direction" not in self.params:
            found_existing_depth_direction = False
            for metric_name, metric_params in self.params["metric_params"].items():
                if "depth_direction" in metric_params:
                    self.params["depth_direction"] = metric_params["depth_direction"]
                    found_existing_depth_direction = True
                    break
            if not found_existing_depth_direction:
