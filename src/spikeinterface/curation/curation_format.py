    ...
    **job_kwargs : dict
    ...
    if len(curation_model.splits) > 0 or len(curation_model.discard_spikes) > 0:
        ...
        for unit_id in curation_model.unit_ids:
            ...
            # If there are spikes to discard, find these, and treat them as a "split".
            # Put the "discarded split" at the start of the "split" unit
            # Hence all `discard_spikes_unit_ids` are in `split_units` and the spikes we want
            # to discard are the first group in the split.
